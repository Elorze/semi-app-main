    ...
    <div class="flex flex-col container-size h-[100vh] rounded-xl bg-[var(--ui-bg)] shadow-lg px-4 sm:px-8 py-8 banner">
        ...
        <div class="w-full flex justify-center gap-18 my-4">
            ...
            <div class="flex flex-col items-center cursor-pointer" @click="navigateTo('/nft')">
                ...
                <span class="text-sm mt-1">展览</span>
            </div>
        </div>
...
onMounted(async () => {
    try {
        ...
        // 使用用户数据中的 Safe Account 地址 测试
        // 这样所有页面都会使用相同的地址
        const safeAddress = user.value?.evm_chain_address as `0x${string}`
        
        console.log('🏠 主页面余额查询调试信息:')
        console.log('使用的地址:', safeAddress)
        console.log('用户数据中的地址:', user.value?.evm_chain_address)
        ...
        console.log('主页面查询到的余额:', data.balance)
        console.log('主页面余额 (ETH):', Number(formatEther(data.balance)))
        console.log('=====================================')
